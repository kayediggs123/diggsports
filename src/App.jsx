...
// March Madness bracket matchups: seed vs seed in round 1
...
export default function MarchMadnessAuction() {
        ...
        <div style={S.setupContainer}>
          ...
          <div style={S.setupColumns}>
            ...
            <div style={S.setupCard}>
              ...
              <p style={S.cardSubtitle}>Set up and run an auction draft</p>
  ...
  if (phase === "done") {
    ...
    // Bracket rendering helper
    const BracketSlot = ({ region, seed }) => {
      const owner = getOwner(region, seed);
      const label = getTeamLabel(region, seed);
      return (
        <div style={{ ...S.bracketSlot, borderLeftColor: owner ? owner.color : "rgba(255,255,255,0.1)" }}>
          <span style={{ ...S.bracketSeed }}>{seed}</span>
          <span style={{ ...S.bracketTeam, color: owner ? "#e8e6e1" : "#3e4a5e" }}>{label}</span>
          {owner && <span style={{ ...S.bracketOwner, color: owner.color }}>{owner.name}</span>}
        </div>
      );
    };

    const BracketMatchup = ({ region, seedA, seedB }) => (
      <div style={S.bracketMatchup}>
        <BracketSlot region={region} seed={seedA} />
        <div style={S.bracketVs}>vs</div>
        <BracketSlot region={region} seed={seedB} />
      </div>
    );
      ...
      <div style={S.page}><style>{globalCSS}</style>
        ...
        {doneTab === "bracket" && (
          <div style={S.bracketContainer}>
            {REGIONS.map((region) => (
              <div key={region} style={S.bracketRegion}>
                <div style={{ ...S.bracketRegionTitle, color: REGION_COLORS[region] }}>
                  <div style={{ width: 10, height: 10, borderRadius: "50%", backgroundColor: REGION_COLORS[region] }}></div>
                  {region.toUpperCase()} REGION
                </div>
                <div style={S.bracketMatchups}>
                  {BRACKET_MATCHUPS.map(([a, b], idx) => (
                    <BracketMatchup key={idx} region={region} seedA={a} seedB={b} />
                  ))}
                </div>
              </div>
            ))}
...
const S = {
  ...
  // Bracket
  bracketContainer: { padding: "24px 24px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 },
  bracketRegion: { background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, padding: 18, overflow: "hidden" },
  bracketRegionTitle: { fontFamily: "'Oswald', sans-serif", fontSize: 14, fontWeight: 700, letterSpacing: 4, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 },
  bracketMatchups: { display: "flex", flexDirection: "column", gap: 8 },
  bracketMatchup: { background: "rgba(255,255,255,0.02)", borderRadius: 8, overflow: "hidden", border: "1px solid rgba(255,255,255,0.04)" },
  bracketSlot: { display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderLeft: "3px solid" },
  bracketSeed: { fontFamily: "'Oswald', sans-serif", fontSize: 11, fontWeight: 700, color: "#5a6478", width: 22, textAlign: "center", flexShrink: 0 },
  bracketTeam: { fontFamily: "'Oswald', sans-serif", fontSize: 13, fontWeight: 600, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
  bracketOwner: { fontFamily: "'Source Sans 3', sans-serif", fontSize: 11, fontWeight: 700, flexShrink: 0 },
  bracketVs: { textAlign: "center", fontSize: 9, color: "#3e4a5e", fontFamily: "'Oswald', sans-serif", letterSpacing: 2, padding: "1px 0", background: "rgba(255,255,255,0.02)" },
  bracketLegend: { gridColumn: "1 / -1", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "16px 20px" },
  // Stats
